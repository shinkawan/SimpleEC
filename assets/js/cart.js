jQuery(document).ready(function ($) {
    ...
    function updateItemSimulator($item) {
        ...
        // --- Member Discount Logic (v3.12.3 Restore) ---
        var isSub = (format === 'subscription');
        if (typeof photoPurchase !== 'undefined' && photoPurchase.is_logged_in && photoPurchase.member_discount_rate > 0 && !isSub) {
            var discountTotal = Math.floor(total * (1 - (photoPurchase.member_discount_rate / 100)));
            $priceDisplay.html('<span style="font-size:0.75em; text-decoration:line-through; opacity:0.6; margin-right:8px;">' + total.toLocaleString() + '</span>' + discountTotal.toLocaleString());
            
            if ($item.find('.member-price-badge').length === 0) {
                $priceDisplay.after('<span class="member-price-badge" style="background:#e0f2fe; color:#0369a1; font-size:10px; padding:2px 6px; border-radius:4px; margin-left:8px; vertical-align:middle; font-weight:normal;">会員割引適用</span>');
            }
        } else {
            $priceDisplay.text(total.toLocaleString());
            $item.find('.member-price-badge').remove();
        }
    }
    ...
    // Modified Add to Cart to include gift_wrap
    ...
    $(document).on('click', '.buy-again-btn', function(e) {
        ...
        if (!items || !Array.isArray(items)) {
            console.error('No items found for reorder');
            return;
        }
